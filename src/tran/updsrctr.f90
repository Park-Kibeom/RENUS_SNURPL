...
!   determine initial parameters
    ...
    integer                 :: m,l,k,mp,ms,m2,idir,idirz
...
!                chibetapf(m,l,k)=xschif(m,l,k)*(1-betat(l,k))+xschifd(m,l,k)*wnp1    ! added in ARTOS ver. 0.2 . 2012_08_06 by SCB    
            ...
            if(exptheta) then
!                phif(:,l,k)=phif(:,l,k)*expof(:,l,k)    ! 2012_08_22 . scb
                do m2=1,ng2
                    phi(m2,l,k)=0
                    do m=mgb(m2),mge(m2)
            ...
            do m2=1,ng2
                aphi=am(indm24(m2,FAST),l,k)*phi(FAST,l,k)+                     &
                     am(indm24(m2,THERMAL),l,k)*phi(THERMAL,l,k)
                     
                do idir=1,nrdir2
                    aphi=aphi+ccr(idir,m2,l,k)*phi(m2,neibr(idir,l),k)
                enddo               
                do idirz=1,2 ! bottom and up
                    aphi=aphi+ccz(idirz,m2,l,k)*phi(m2,l,neibz(idirz,k))
                enddo
            ...
            if(exptheta) then
                phid(:,l,k)=phi(:,l,k)
!                phi(:,l,k)=phi(:,l,k)*expo(:,l,k)    ! 2012_08_22 . scb
            endif        
        endif
