...
! Set CMFD Linear System for Hexagonal Geometry
	    ...
	    use tpen_sp3
	    use xsec
	    use cmfdhex2g, only : dcmat, cmat, dsum, dfd, dhat, dfdz, dhatz !, rhzbar2 - doesn't exist in module, alc_06.01.16
      ...
      real :: tmpm(6)
	    ...
	    integer :: k, m, l, i, idir
	    real :: vol, chi1, chi2, chip, chid
      ...
      if(iroute.eq.2) then
         do iz=1,nz
            do ih=1,nassy
            vol=volnode(ih,iz)
            chi1=reigvs*xschi(1,ih,iz)
            chi2=reigvs*xschi(2,ih,iz)
            dcmat(1,ih,iz)=(xst(1,ih,iz)-chi1*xsnf(1,ih,iz) &
                           +dsum(1,ih,iz))*vol
            dcmat(2,ih,iz)=-chi1*xsnf(2,ih,iz)*vol-xss(2,1,ih,iz)*vol 
